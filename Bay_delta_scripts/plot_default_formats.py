...
__all__ = ['set_color_cycle_dark2', 'set_dual_axes', 'set_elev_dual_axes', 'set_salt_dual_axes']
...
def set_dual_axes(ax1, ts):
    ...
    if 'unit' in ts.props.keys():
        ...
        if unit == 'm' or unit == 'meter':
            ...
            return ax2
    print "Warning: set_dual_axes: No supported unit in the time series."
    return None

def set_elev_dual_axes(ax1, names=None):
...
def create_second_axis(ax1, y_converter_f=None,
    ...
    if y_converter_f:
        ax2.set_ylim(y_converter_f(numpy.array(ax1.get_ylim())))
    else:
