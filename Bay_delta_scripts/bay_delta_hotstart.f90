...
! ifort -O2 -CB -Bstatic -o bay_delta_hotstart bay_delta_hotstart.f90

!   Input: 
!     (1) synoptic_cruise_station.txt and synoptic_cruise_salinity_temperature.txt (USGS survey data);
!     (2) hgrid.gr3;
!     (3) vgrid.in; 
...
!      implicit none
 ...
 !     integer, parameter :: debug=1
      ...
      program bay_delta_hotstart
        ...
        if  ( iargc() .lt. 2) then
             write(*,*) 'usage:'
             write(*,*) 'bay_delta_hotstart  --station station.txt --delta_salt 0.1  salt_temp.txt'
             write(*,*) 'bay_delta_hotstart  -s station.txt -d 0.1  salt_temp.txt'
             write(*,*) ' delta_salt input is optional, default 0.15'
             stop
        end if
        
        opts(1) = option_s( "station", .true., 's' )
        opts(2) = option_s( "delta_salt",  .true.,  'd' )
        opts(3) = option_s("help",.true.,'h')
        do
             select case( getopt( "s:d:h", opts ))
                 case( char(0))
                     exit
                 case( 's' )
                     stationfile=adjustl(optarg)
                 case( 'd' )
                     read(optarg,*) deltasalt
                 case('h')
                     write(*,*) 'usage:'
                     write(*,*) 'bay_delta_hotstart  --station station.txt --delta_salt 0.05  salt_temp.txt'
                     write(*,*) 'bay_delta_hotstart  -s station.txt -d 0.05  salt_temp.txt'
                     write(*,*) ' delta_salt input is optional, default 0.15'
                     stop
                 case( '?' )
                     ...
                     stop
                 case default
                     tempsaltfile=adjustl(optarg)
                     exit
             end select
        end do
          
        if (stationfile.eq."invalid") then
            print *, "you must provide station file"
        else if ((tempsaltfile.eq."invalid") .or. (tempsaltfile.eq.""))then
            print *, "The last input must be a valid salt and temp survey data file "
        else
            ...
            call hotstart_from_cruise_data(tempsaltfile,stationfile,initialelevfile,&
                 hgridfile,vgridfile,estuaryfile,deltasalt,hotstartfile)
        end if
        
      end program bay_delta_hotstart
...
!     Read in survey data
      ...
      do ! i=1,549 !read only 03/12 data
        if(i<=2) then
          ...
          mm=-9999
          dd=-9999
100       format (i2,a1,i2,a1,i4,a1,i4,a1,a)
          read(9,100,iostat=code) mm,c1,dd,c2,yyyy,c3,tttt,c4,tempstring
          
          ! reach the end of file
          if (iostat.ne.0) then
             if (i.eq.3) then
               write(*,*) "input data is empty, no result generated"
               stop
...
!     MPI SELFE
      ...
      stop
    end  subroutine hotstart_from_cruise_data
