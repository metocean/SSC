## This module contains polygon data holders and related opertaions.
##
...
class Polygon(object):
    ...
    def __init__(self, points = None, name = None,
        ...
        """ Constructor
            ...
            attribute:
                Value associated with the polygon
        """
    ...
    def check_point_inside_polygon(self, pt):
        """ Check if points are in this polygon.
    ...
    def box(self):
        """ Provide a bounding box of this polygon.
            return = a numpy array that contains [x_min, x_max, y_min, y_max]
        """
#        _box = np.array([np.min(self._p,axis=0),
#                         np.max(self._p,axis=0)])

        _box=np.concatenate((np.min(self._p,axis=0),np.max(self._p,axis=0)),)
	_box=_box[[0,2,1,3]]
