...
def stacked_dem_fill(files,points,negate=False,require_all = True, na_fill=None):
    '''
    ...
    files:  list of files. Existance is checked and ValueError for bad file
    points: this is a numpy array of points, nrow = # points and ncol = 3 (x,y,z). The z column is filled in-place
    ...
    for infile in files:
        ...
        dem[dem == nd] = np.nan
        val = bilinear(points[np.isnan(points[:,2]),0:2], gt, dem)
        val[val < -20000] = np.nan
        points[np.isnan(points[:,2]),2] = val
        indataset = None
        dem = None
        rb = None
    ...
    if any(np.isnan(points[:,2])):
        f = open("dem_misses.txt","w")
        for p in points[np.isnan(points[:,2]),:]:
            f.write("%s,%s\n"  % tuple(p[0:2]))
        ...
        if require_all:
            ...
            print message
    points[np.isnan(points[:,2]),2] = na_fill
    return points
...
def bilinear(points, gt, raster): 
    ...
    nearx = (np.abs(inboundx - 6.25143225e+005) < 1.) & (np.abs(inboundy-4.20178364e+006) < 1.)
...
def test_main():
    ...
    print stacked_dem_fill(["bay_delta_mini.tif","bay_delta_west.tif","bay_delta_east.tif"], points)
...
def filelist_main(demlistfile,pointlistfile,sep=""):
    ...
    filelist = [f.strip() for f in open(demlistfile,"r").readlines() if (f and len(f) > 1)]
    bad_files = [f for f in filelist if not os.path.exists(f)]
    if len(bad_files) > 0:
        for file in bad_files:
            print "Does not exist: %s " % file
    ...
    points[:,2] = np.nan
    print points
    points = stacked_dem_fill(filelist,points)
    print points
    return points


    
def fill_2dm(infile, outfile, demlist):
    import string
    ...
    endndx = next((i for i, v in enumerate(all_lines[firstndx:]) if not v.startswith("ND")), -1) + firstndx
    pts = np.array([[float(x) for x in string.split(s.strip())[2:5]] for s in all_lines[firstndx : endndx]])
    pts[:,2] = np.nan
    files = [x.strip() for x in open(demlist,"r").readlines() if x and len(x) > 1 and not x.startswith("#")]    
    pts = stacked_dem_fill(files, pts, require_all = False,na_fill = DEFAULT_NA_FILL)
    
    all_lines[firstndx:endndx] = ("ND %i  %18.8f %18.8f  %18.8f\n" % ((i+1),pt[0],pt[1],pt[2]) for i,pt in enumerate(pts))
    fout = open(outfile,"w")
    for line in all_lines:
        fout.write(line)
    return None

def fill_gr3(infile, outfile, demlist,elev2depth=True):
    import string
    import math
    print "Filling elevations in %s and writing to %s" %(infile,outfile)
    f = open(infile,'r')
    ...
    firstndx = 2
    stopndx = nnode+2
    pts = np.array([[float(x) for x in string.split(s.strip())[1:4]] for s in all_lines[firstndx:stopndx]])
    pts[:,2] = np.nan    
    files = [x.strip() for x in open(demlist,"r").readlines() if x and len(x) > 1 and not x.startswith("#")]       
    pts = stacked_dem_fill(files, pts, require_all = False,na_fill = DEFAULT_NA_FILL, negate = elev2depth)
    padding = 2
    ...
    all_lines[firstndx:stopndx] = (nfmt % ((i+1),pt[0],pt[1],pt[2]) for i,pt in enumerate(pts))
...
def create_arg_parser():
    import argparse
    parser = argparse.ArgumentParser(description='Fill node elevations in a *.2dm SMS mesh or gr3 file using a prioritized list of DEMs.')
    ...
    parser.add_argument('--elev2depth',action = 'store_true', default=False,help='Convert elevation to depth by flipping sign. This is typical when using gr3 format, less so with 2dm.')
    return parser
...
if __name__ == "__main__":
    ...
    if infile.endswith(".2dm"):
        if elev2depth: print "Warning: --elev2depth is an unusual option for 2dm files"
        fill_2dm(bakfile,infile,demlistfile)
    elif infile.endswith(".gr3"):
        if not elev2depth: print "Warning: omitting --elev2depth is an unusual option for gr3 files"    
        fill_gr3(bakfile,infile,demlistfile,elev2depth)
    else:
