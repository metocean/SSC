...
def create_arg_parser():
    ...
    parser.add_argument('--overwrite',type = bool,default = False, help = 'Overwrite existing files (if False they will be skipped, presumably for speed')
    return parser
...
def cdec_download(stations,dest_dir,start,end="Now",param="ec",overwrite=False):
    ...
    for station,p,z in zip(stations,param,paramcode):
        print "Downloading station: %s param: %s" % (station,p)
        ...
        found = False
        for code in z:
            station_query_base = "http://cdec.water.ca.gov/cgi-progs/queryCSV?station_id=%s&sensor_num=%s&dur_code=%s&start_date=%s&end_date=%s" 
            dur_codes = ["E","H","D"]            
            for dur in dur_codes:
                station_query = station_query_base % (station,code,dur,stime,etime)
                #print station_query
