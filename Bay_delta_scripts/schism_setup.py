...
class SchismSetup(object):
    ...
    def write(self, dir, grid_fname = "hgrid.gr3",
        ...
        """
        if self._verbose > 0:
            print "Writing SCHISM input files..."
        # Grid output
        gr3_writer = Gr3IO()
        ...
        if self._verbose > 0:
            print "Done writing SCHISM input files."
        ...
        for name, line in lines.iteritems():
            ...
            for node_i in up_path:
                if not flags[node_i] == -1:
                    print "This node is flagged already: ", node_i, \
                          flags[node_i], count
            ...
            if len(down_path) < 1 or len(up_path) < 1:
                print "No nodes in the path..., line: ", line
...
#             struct.use_timeseries = int(structure['use time series'])
    ...
    def _partition_nodes_with_polygons(self, polygons, default):
        ...
        if empty:
            print "Warning: This polygon contains no nodes: ", poly.name
    ...
    def _interpolate(self, times, data, dt_out, out_fname):
        ...
        print "Interpolating to a new series of size %s" % nout
        ...
        print "number of loops required to convert the file: %s" %iloop
        print "steps per loop: %s" %loop_step

        for j in range(0,iloop):
            print "working on loop %s out of %s" % (j+1, iloop)
    ...
    def _read_flow_xsects(self, fname):
        ...
        for l in f:
            ...
            tokens = l.split(',')
            if len(tokens) < 8:
                print "Warning: Not enough items..."
                raise Exception()
    ...
    def trim_to_left_of_mesh(self, line_segments):
        ...
        self.mesh.clear_boundary()
        print "Removed the old boundary information."
