...
from vtools.data.vtime import infer_interval as vt_infer_interval
...
__all__ = ['read_ts', 'read_noaa', 'read_wdl', 'read_cdec', 'read_usgs']
...
class TextTimeSeriesReader(object):
    ...
    def read(self, fpath, start=None, end=None, force_regular=True, selector=None):
        ...
        with open(fpath, 'r') as f_in:
            ...
            if n_headerlines>0: 
                for i in range(n_headerlines): f_in.readline()
        ...
        if force_regular:
            interval = vt_infer_interval(times[:11],
                                         fraction=0.5,standard=[vtt.minutes(6),vtt.minutes(10),vtt.minutes(15),vtt.hours(1)])
            if not interval:
                for t in times[:10]:
...
class CDECReader(TextTimeSeriesReader):
    ...
    def __init__(self):
        ...
        self._header_approval_regexes = [r"Title:\s\"[A-Z0-9]{3}\.csv\""]
...
class USGSRdbReader(TextTimeSeriesReader):
    ...
    def process_header(self, fpath,selector=None):
        ...
        assert zone_col > time_col     # Needed for accounting because datetime column has a space

        data_col_re = re.compile(r"(\d{2}_\d{5})|value$")
        data_names = []
        data_ndxs = []
