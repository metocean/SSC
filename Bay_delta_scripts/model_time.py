...
def create_arg_parser():
    ...
    parser_elapsed = subparsers.add_parser('to_elapsed', help='Convert input datetime string or *.th file with datetime column to equivalent output in elapsed seconds.')
    parser_elapsed.add_argument('dated_input', default = None, nargs = '+', help = 'One or more dates to be converted in ISO-like format 2009-03-12T00:00:00 (time can be ommitted) or the name of a *.th file with the time column in this format.')
    
    parser_elapsed.add_argument('--start',required=False, type = str,help = 'Starting date and time basis for output if the input is a file. Allows a larger database with readable times to be clipped')
    parser_elapsed.add_argument('--annotate', default = False, action = 'store_true')    
    parser_elapsed.set_defaults(func = to_elapsed)
    parser_elapsed.add_argument('--step', default = None, nargs = '?', type=float, help = 'Model time step. If given, answer will be the integer time step.') 
    ...
    parser_date = subparsers.add_parser('to_date', help='Convert input elapsed seconds or *.th file with elapsed seconds as the time column to equivalent output with a datetime or annotated with datetimes.')
    parser_date.add_argument('--start',required=True, type=str,help = 'Start time in ISO-like format 2009-03-12T00:00:00. Time part is optional.')
    parser_date.add_argument('--step', default = None, nargs = '?', type=float, help = 'Model time step. If given, answer will be the integer time step.')     
    parser_date.add_argument('elapsed_input', default = None, nargs = '+', help = 'One or more numbers representing elapsed seconds since the start argument or the name of a *.th file with the time column in elapsed seconds.')
    ...
    parser_clip = subparsers.add_parser('clip', help='Clip (subset) an input file in elapsed time to a new, later, start date')    

    parser_clip.add_argument('--start',required=True, help = 'Start time in ISO-like format 2009-03-12T00:00:00. Time part is optional.')
    parser_clip.add_argument('--clip_start',required=True,help = 'Starting date for output.')
    parser_clip.add_argument('--out', default = None, type=str,
    help='Name of output file. If input is a *.th file the file will be converted and output to this file, otherwise printed to screen')
    parser_clip.add_argument('elapsed_input', default = None, nargs = '+', help = 'One or more numbers representing elapsed seconds since the start argument or the name of a *.th file with the time column in elapsed seconds.')    
...
def to_elapsed(args):
    ...
    annotate = not (to_elapsed or to_date)

    
    # Delegate chore based on the input (file, timestamp string or elapsed time)
    if th:
        infile=input[0]
        if to_elapsed:
            file_to_elapsed(infile,s,outpath)
    else:
        describe_timestamps(input,s,dt)
...
def clip(args):
    ...
    snew=args.clip_start
    input=args.elapsed_input

    # convert start time string input to datetime
    start=datetime.datetime(*map(int, re.split('[^\d]', args.start))) 
    scliptime=datetime.datetime(*map(int, re.split('[^\d]', args.start)))
    ...
    with open(infile,"r") as thfile:
        ...
        for line in thfile:       
            if line and len(line) > 1:
                ...
                exact = msec == 0.0
                outline = line.replace(timestr,"%-10.1f " % msec_orig) 
                use = (msec >= 0.0)
...
def file_to_elapsed(infile,start,outpath=None,annotate=False):
    if type(start) == str:
...
def describe_elapsed(times,start,dt=None):
    ...
    for elapsed in times:
        ...
        if elapsed.endswith("d") or elapsed.endswith("days") or elapsed.endswith("day"):
            ...
            msec = float(elapsed)    # assuming that the input elapsed time is in seconds
        mdelta = datetime.timedelta(seconds = msec)
